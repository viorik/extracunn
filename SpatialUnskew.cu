...
__global__ void SpatialUnskew_updateOutput(
...
static int extracunn_SpatialUnskew_updateOutput(lua_State *L)
{
  ...
  SpatialUnskew_updateOutput<<<gridSize, blockSize, 0, THCState_getCurrentStream(state)>>>(
    devInput, devOutput);

  return 1;
}

__global__ void SpatialUnskew_updateGradInput(
...
static int extracunn_SpatialUnskew_updateGradInput(lua_State *L)
{
  THCState *state = getCutorchState(L);
  // Inputs
  THCudaTensor *input = (THCudaTensor *)luaT_checkudata(L, 2, "torch.CudaTensor");
  ...
  SpatialUnskew_updateGradInput<<<gridSize, blockSize, 0, THCState_getCurrentStream(state)>>>(
    devGradInput, devGradOutput);
 
  return 1;
}

static const struct luaL_Reg extracunn_SpatialUnskew__ [] = {
  {"SpatialUnskew_updateOutput", extracunn_SpatialUnskew_updateOutput},
  {"SpatialUnskew_updateGradInput", extracunn_SpatialUnskew_updateGradInput},
  {NULL, NULL}
};
...
void extracunn_SpatialUnskew_init(lua_State *L)
{
  ...
  luaT_registeratname(L, extracunn_SpatialUnskew__, "nn");
  lua_pop(L,1);
}
