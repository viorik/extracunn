...
__global__ void SpatialMirrorHorizontal_updateOutput(
...
static int extracunn_SpatialMirrorHorizontal_updateOutput(lua_State *L)
{
  ...
  SpatialMirrorHorizontal_updateOutput<<<gridSize, blockSize, 0, THCState_getCurrentStream(state)>>>(
    devInput, devOutput);

  return 1;
}

__global__ void SpatialMirrorHorizontal_updateGradInput(
...
static int extracunn_SpatialMirrorHorizontal_updateGradInput(lua_State *L)
{
  THCState *state = getCutorchState(L);
  // Inputs
  THCudaTensor *input = (THCudaTensor *)luaT_checkudata(L, 2, "torch.CudaTensor");
  ...
  SpatialMirrorHorizontal_updateGradInput<<<gridSize, blockSize, 0, THCState_getCurrentStream(state)>>>(
    devGradInput, devGradOutput);

  return 1;
}

static const struct luaL_Reg extracunn_SpatialMirrorHorizontal__ [] = {
  {"SpatialMirrorHorizontal_updateOutput", extracunn_SpatialMirrorHorizontal_updateOutput},
  {"SpatialMirrorHorizontal_updateGradInput", extracunn_SpatialMirrorHorizontal_updateGradInput},
  {NULL, NULL}
};
...
void extracunn_SpatialMirrorHorizontal_init(lua_State *L)
{
  ...
  luaT_registeratname(L, extracunn_SpatialMirrorHorizontal__, "nn");
  lua_pop(L,1);
}
