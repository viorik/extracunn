...
static int extracunn_SpatialConvolutionMMNoBias_updateOutput(lua_State *L) {
  ...
  THAssert(THCudaTensor_checkGPU(state, 4, input, output, weight,
                                 columns));
  ...
  //THCudaTensor_fill(state, output_n, 0);
...
static int extracunn_SpatialConvolutionMMNoBias_updateGradInput(lua_State *L) {
  THCState *state = getCutorchState(L);
  // Inputs
  THCudaTensor *input = (THCudaTensor *)luaT_checkudata(L, 2, "torch.CudaTensor");
...
static int extracunn_SpatialConvolutionMMNoBias_accGradParameters(lua_State *L) {
  THCState *state = getCutorchState(L);
  // Inputs
  THCudaTensor *input = (THCudaTensor *)luaT_checkudata(L, 2, "torch.CudaTensor");
  THCudaTensor *gradOutput = (THCudaTensor *)luaT_checkudata(L, 3, "torch.CudaTensor");
  ...
  THAssert(THCudaTensor_checkGPU(state, 4, input, gradOutput, gradWeight,
                                 columns));
  luaL_argcheck(L, input->nDimension == 3 || input->nDimension == 4, 2, "3D or 4D (batch mode) tensor is expected");
...
static const struct luaL_Reg extracunn_SpatialConvolutionMMNoBias__ [] = {
  {"SpatialConvolutionMMNoBias_updateOutput", extracunn_SpatialConvolutionMMNoBias_updateOutput},
  {"SpatialConvolutionMMNoBias_updateGradInput", extracunn_SpatialConvolutionMMNoBias_updateGradInput},
  {"SpatialConvolutionMMNoBias_accGradParameters", extracunn_SpatialConvolutionMMNoBias_accGradParameters},
  {NULL, NULL}
};
...
void extracunn_SpatialConvolutionMMNoBias_init(lua_State *L)
{
  ...
  luaT_registeratname(L, extracunn_SpatialConvolutionMMNoBias__, "nn");
  lua_pop(L,1);
}
